...
async function cloneRepo() {
    ...
    const zipPath = path.join(__dirname, 'SUBZERO-BOT-main.zip');
    ...
    try {
        ...
        const extractedDir = path.join(extractPath, 'SUBZERO-BOT-main');
        fs.readdirSync(extractedDir).forEach(file => {
            fs.renameSync(path.join(extractedDir, file), path.join(extractPath, file));
        });
        ...
        console.log('Repository cloned and extracted successfully.');
    } catch (error) {
...
if(!config.SESSION_ID) return console.log('Please add your Subzero session to SESSION_ID env !!')
...
const port = process.env.PORT || 9090;
 ...
 async function connectToWA() {
  ...
  conn.sendMessage(conn.user.id, { image: { url: `https://i.ibb.co/GMFDmjq/mrfrankofc.jpg` }, caption: up })
  }
  })
 conn.ev.on('creds.update', saveCreds)  
  ...
  const udp = botNumber.split('@')[0];
    const jawad = ('263719647303', '18062212660', '263788236690');
